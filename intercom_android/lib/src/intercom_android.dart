...
import 'package:intercom_android/src/intercom_api.g.dart';
import 'package:intercom_platform_interface/intercom_platform_interface.dart';

class AndroidIntercom extends IntercomPlatform {
  final AndroidIntercomApi _intercomApi = AndroidIntercomApi();

  /// Registers this class as the default instance of [IntercomPlatform].
  static void registerWith() {
    IntercomPlatform.instance = AndroidIntercom();
  }
  ...
  Future<void> initialize({required String appId, String? androidApiKey, String? iosApiKey}) {
    if (androidApiKey != null && androidApiKey.isNotEmpty) {
      return _intercomApi.initialize(appId, androidApiKey);
    }
    throw ArgumentError('androidApiKey can not be empty or null');
  }
  ...
  EventChannel _eventChannelFor() {
    return const EventChannel('dev.flutter.pigeon.AndroidIntercomApi.getUnreadStream');
  }
}
